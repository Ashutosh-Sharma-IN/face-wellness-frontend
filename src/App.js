  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
    <GlassCard className="max-w-md w-full p-8" hover={false}>
      <div className="text-center mb-6">
        ...
        <h3 className="text-xl font-bold text-white mb-2">Responsible Use Guidelines</h3>
      </div>
      
      <div className="space-y-4 text-sm text-slate-300 mb-6">
        <div className="flex gap-3">
          <span className="text-emerald-400">✓</span>
          <p>Only upload <strong className="text-white">your own face photos</strong> for personal wellness tracking</p>
        </div>
        <div className="flex gap-3">
          <span className="text-emerald-400">✓</span>
          <p>Photos must be <strong className="text-white">appropriate and non-offensive</strong></p>
        </div>
        <div className="flex gap-3">
          <span className="text-rose-400">✗</span>
          <p>Do not upload photos of <strong className="text-white">others without consent</strong></p>
        </div>
        <div className="flex gap-3">
          <span className="text-rose-400">✗</span>
          <p>No <strong className="text-white">inappropriate, harmful, or adult content</strong></p>
        </div>
        <div className="flex gap-3">
          <span className="text-cyan-400">ℹ</span>
          <p>Face detection is <strong className="text-white">required</strong> - only valid face photos will be analyzed</p>
        </div>
      </div>
      
      <p className="text-xs text-slate-500 mb-6 text-center">
        By continuing, you agree to use this app responsibly. Misuse may result in account suspension.
      </p>
      
      <button
        onClick={onAccept}
        className="w-full py-3 rounded-xl font-semibold bg-gradient-to-r from-cyan-500 to-purple-600 text-white
...
function App() {
  ...
  // Connect video stream and start face detection
  useEffect(() => {
  ...
  // For production, consider using TensorFlow.js face-detection or similar
  const startFaceDetection = useCallback(() => {
    ...
    const detectFace = () => {
      ...
      if (video.readyState >= 2 && video.videoWidth > 0) {
        // Simple heuristic: check if there's significant content in the center region
        ...
        // Check for skin-tone like colors in center
        let skinPixels = 0;
        ...
        const skinPercentage = skinPixels / (100 * 100);
        setFaceDetected(skinPercentage > 0.15); // At least 15% skin-tone pixels
      }
  ...
  const capturePhoto = () => {
    ...
    if (!faceDetected) {
      showNotification('Please position your face in the frame', 'warning');
      return;
    }
    ...
    canvas.toBlob((blob) => {
      if (blob) {
        ...
        // Create cropped preview URL that matches the camera view
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        ...
        <div className="max-w-md w-full">
          <div className="text-center mb-12 animate-fade-in">
            ...
            <h1 className="text-4xl font-bold text-white mb-3 tracking-tight">Face Wellness</h1>
            <p className="text-slate-400 text-lg">Track how your daily habits reflect on your face</p>
          </div>
          ...
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="text-center">
              <p className="text-slate-500 text-sm">
                🔒 Face detection required • Only appropriate face photos allowed
              </p>
            </div>
          </div>
        )}
        ...
        {currentView === 'camera' && (
          <div className="max-w-lg mx-auto animate-fade-in">
            <GlassCard className="p-6 overflow-hidden" hover={false}>
              ...
              <p className="text-slate-400 text-sm text-center mb-6">Position your face clearly in good lighting</p>
              ...
              {!cameraStream && !capturedImage && (
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-400 mb-2">Natural photos only • No filters</p>
                  <p className="text-slate-500 text-sm mb-6">Best results with even lighting on your face</p>
                  <button
                    onClick={startCamera}
              ...
              {cameraStream && !capturedImage && (
                <div className="space-y-4">
                  <div className="relative rounded-2xl overflow-hidden bg-black aspect-[3/4]">
                    ...
                    <div className={`absolute top-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full text-sm font-medium
                      ${faceDetected 
              ...
              {/* Photo Preview - FIXED: Show same framing as camera */}
                ...
                <div className="space-y-4">
                  <div className="relative rounded-2xl overflow-hidden bg-black aspect-[3/4]">
                    <img 
                      ...
                      className="w-full h-full object-cover"
                    />
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-emerald-500/80 text-white text-sm font-medium">
                      ✓ Photo captured
                    </div>
                  </div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-4 gap-2 text-sm">
                        <div className="text-center">
                          ...
                          <p className="text-white font-semibold">{item.results?.eye_pouch?.value ?? '—'}</p>
                        </div>
                        <div className="text-center">
                          <p className="text-slate-400">Dark</p>
                          <p className="text-white font-semibold">{item.results?.dark_circle?.value ?? '—'}</p>
                        </div>
                        <div className="text-center">
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-white font-semibold">{item.results?.forehead_wrinkle?.value ?? '—'}</p>
                        </div>
                      </div>
                    </div>
                  ))}
